...
async def stream_to_deepgram(device, on_final_callback=None):
    if not DEEPGRAM_API_KEY:
        raise RuntimeError(
            "Manca DEEPGRAM_API_KEY. Impostala come variabile d'ambiente o in un file .env."
        )

    bridge = AudioBridge(device)
    bridge.start()
    print(f"Cattura audio avviata sul device {device}. Ctrl+C per fermare.")
...
def main():
    ...
    try:
        asyncio.run(stream_to_deepgram(device))
